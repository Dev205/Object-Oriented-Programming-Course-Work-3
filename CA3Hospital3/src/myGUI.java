...
//	Database db = new Database();
	Database db = null; // it works like this
...
//	DoubleLinkedList dll = new DoubleLinkedList();
	...
	public myGUI() {
		...
		// Instantiate double linked list here, if instantiated in submit, a new list will be created each time button is pressed
		...
		btnSubmit.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
		        ...
		        dll.add(patient, 0); // Nurse will update the priority later
		        
		        
		        db.createPatientsDB(); //establish the connection with the database
		        db.savePatientData(patient); // saves patient to the database
		...
		Save_Nurse.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				//dll.getNode(Integer.valueOf(Index_Nurse.getText().toString())).setPriority(Integer.valueOf(priorityTextField.getText().toString()));

				// sets the priority of the Node from the spinner
				dll.getNode(Integer.valueOf(Index_Nurse.getText().toString())).setPriority(Integer.valueOf((Integer) spinner.getValue()));

				// finds the patient associated with the Node
				Patient patient=(Patient) dll.getNode(Integer.valueOf(Index_Nurse.getText().toString())).getData(); 
				
				// sets the vital signs of the patient
				patient.setVitalSigns(vitalSignstextField.getText().toString());
				db.updatePatientNurse(patient);  // update the patient's vital sign in the database
				...
				//testing this out here, it seems to work here, wait, no it doesnt might come back to this, not a big problem
//				if(dll.size() <= dll.size()+1){ // because the nurse has just filled in one patient detials, so there is at least on in the queue
//					
//					firstname_Nurse.setText("no patients in queue");
//					lastname_Nurse.setText("no patients in queue");
//	
//				}
		...
		Next_Nurse.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				//System.out.println(dll.toString()); // is printing out as expected
//				if(dll.size() <= 0){ // because the nurse has just filled in one patient detials, so there is at least on in the queue
//					
//					firstname_Nurse.setText("no patients in queue");
//					lastname_Nurse.setText("no patients in queue");
//				}else{

		        for (int i = 1; i <= dll.size(); i++) { // iteratees through each node 
		            if (dll.getNode(i).getPriority() == 0) {
		            	Patient patient=(Patient) dll.getNode(i).getData(); 
		...
		Next_Doctor.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(dll.size() <= 0){ 
					...
					Patient patient = (Patient) dll.getNode(dll.getHighestPriorityPosition()).getData();
